...
def load_all_datasets(base_dir="generated_datasets"):
    datasets = {}
    base_path = Path(base_dir)

    for dataset_dir in sorted(base_path.glob("dataset_*")):
        try:
            with open(dataset_dir / "data.pkl", "rb") as f:
                X_train, X_test, y_train, y_test = pkl.load(f)

            dataset_name = dataset_dir.name  # e.g., "dataset_003"
            datasets[dataset_name] = (X_train, X_test, y_train, y_test)

        except Exception as e:
            print(f"Failed to load {dataset_dir}: {e}")
...
bbmm_gpr_param_space = {
    "epochs": lambda trial:trial.suggest_int("epochs", 500, 3000), 
    "learning_rate": lambda trial:trial.suggest_float("learning_rate", 1e-4, 1e-2, log=True)
}
...
UQ_REGRESSORS = {
    ...
    #"TempScaleDeepEns": (temp_scale_deep_ens, temp_scale_deep_ens_param_space, interval_score, False)
}

def test_uqregressor_hyperparam_tuning(output_dir='generated_datasets'):
    datasets = load_all_datasets()
    for dataset_name, (X_train, X_test, y_train, y_test) in datasets.items():
        fm = FileManager(os.path.join(output_dir))
        for reg_name, (reg_class, param_space, score_fn, greater) in UQ_REGRESSORS.items():
            ...
            if reg_name not in ['gpr']: 
                ...
                best_model = reg_class.fit(X_train, y_train)

            mean, lower, upper = best_model.predict(X_test)
            metrics = compute_all_metrics(mean, lower, upper, y_test, alpha=best_model.alpha)
            ...
            # Save best model
            model_path = os.path.join(output_dir, f"{dataset_name}", f"{reg_name}")
            ...
            if reg_name not in ['gpr']: 
                ...
                study_path = os.path.join(output_dir, f"{dataset_name}", f"{reg_name}_optuna_trials.csv")
...
if __name__ == "__main__":
    ...
    test_uqregressor_hyperparam_tuning()
