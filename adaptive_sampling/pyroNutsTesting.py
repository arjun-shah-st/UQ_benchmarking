...
def main(ndim, npoints=1000):
    ...
    sample_inputs = input_sampler.uniformly_sample(batch_samples=10000)
    sample_mean, sample_lower, sample_upper = nn.predict(sample_inputs)
    sample_mean = sample_mean.reshape(-1, 1) 
    y_dim = sample_mean.shape[-1]
    base_dist = StandardNormal(shape=[y_dim])
    transforms = [] 
    num_layers = 4 
    for _ in range(num_layers):
        transforms.append(MaskedAffineAutoregressiveTransform(features = y_dim, hidden_features=2 * y_dim))
    flow = Flow(CompositeTransform(transforms), base_dist)

    optimizer = torch.optim.Adam(flow.parameters(), lr=1e-3)
    n_epochs = 200
    batch_size = 256
    dataset = sample_mean  # shape (N, y_dim)
    ...
    # (Optional) Inspect the learned density
    xs = torch.linspace(float(torch.min(sample_mean)), float(torch.max(sample_mean)), 200).reshape(-1, 1)
    with torch.no_grad():
        logps = flow.log_prob(xs)
    plt.plot(xs.numpy(), torch.exp(logps).numpy(), label="flow density")
    plt.hist(sample_mean.numpy(), bins=50, density=True, alpha=0.5, label="empirical")
    plt.legend()
    plt.show()

    print("largest_residual:", largest_residual.item())

    print(largest_residual)
    def log_pdf(x):
        mean_x, lower, upper = nn.predict(x.unsqueeze(0))
        ...
        log_w = flow.log_prob(mean_x).squeeze()  
