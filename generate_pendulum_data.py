...
def pendulum(n=10000, n_t=10, g_range=None, theta_range=None, ell_range=None, m_range=None, t_spread=None,
    ...
    feat = np.concatenate([ell, t], axis=1)
    y = g
...
if __name__ == "__main__":
    for n in (250, 500, 1000, 2000): 
        feat, y, calc_y, delta_y = pendulum(n=n, n_t=4)
        matrix = np.concatenate([feat, y.reshape((n, 1)), calc_y.reshape((n, 1)), delta_y.reshape((n, 1))], axis=1)
        with open(f"5D_pendulum_{n}.pkl", 'wb') as f: 
            pickle.dump(matrix, f)

        print (f"Saved feature, response matrix for dataset of size {n}, shape {matrix.shape}")
